...
ts_content = r'''import Database from 'better-sqlite3';
...
export class DatabaseService {
  ...
  private seedDefaultData() {
    ...
    // Vehicles
    const vehList = [
      { id: 'veh_1', plateNumber: '粤B12345', vehicleType: '轿车', brand: '丰田凯美瑞', model: '2022款', color: '黑色', driverId: 'emp-7', driverName: '周伟明' },
      { id: 'veh_2', plateNumber: '粤B67890', vehicleType: '商务车', brand: '别克GL8', model: '2023款', color: '白色', driverId: 'emp-7', driverName: '周伟明' },
      { id: 'veh_3', plateNumber: '粤B11111', vehicleType: '货车', brand: '东风货车', model: '4.2米', color: '蓝色', driverId: 'emp-11', driverName: '杨秀英' },
    ];
    for (const v of vehList) {
      this.db.prepare('INSERT INTO vehicles VALUES (@id,@plateNumber,@vehicleType,@brand,@model,@color,@driverId,@driverName,@status,@mileage,@insuranceExpiry,@remark,@createdAt)').run({ ...v, mileage: 10000 + i * 5000 if 'i' in locals() else 10000, insuranceExpiry: '2027-03-01', remark: '', createdAt: now });
    }
...
with open('D:/feida/server/modules/database/database.service.ts', 'w', encoding='utf-8') as f:
    f.write(ts_content)

print(f'Done! File size: {len(ts_content)} bytes')
