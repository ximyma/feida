# 飞达智能 HR 系统 (Feida Intelligent HR System) v1.2.0

> 基于 AI 大模型的**智能人力资源管理 + 企业门户 + 商城 + 低代码平台**一体化全栈平台。
> Apache 2.0 开源。

飞达是一套开箱即用的企业数字化平台：以人力资源管理（HR）为核心，向上集成 **AI 代码助手 / 智能对话 / 知识库 / BI 看板**，横向覆盖 ERP 制造、内容门户（CMS）、在线商城（ShopXO 超集）与**低代码应用平台**，并通过 **RBAC 细粒度权限 + 站点级权限作用域 + 多语言 i18n** 实现企业级管控。
...
| 域 | 模块 | 关键能力 |
|----|------|----------|
| **AI** | 智能对话 / 代码助手 / 知识库 / BI 看板 / 文件上传 | 流式对话、7 工具 (sql_query/grep/read_file/bash/glob/write_file/patch)、文件上传(拖拽/按钮, 文本读取+元数据)、混合检索(RAG)、ECharts+AI 洞察、多提供商 |
| **低代码平台** | 应用构建器 / 模板设计器 / 关联关系 / 运行时视图 | 4 步向导、23 种字段类型、拖拽排序、JSON/Excel 导入字段、many2one 下拉搜索、6 种视图控件、关联关系管理(belongs_to/has_many/many_to_many)、ORM 自动建表、热部署 |
| **人力资源** | 人事 / 考勤 / 薪酬 / 绩效 / 招聘 / 培训 / 统计 | 员工档案+字段自定义、排班、薪资公式引擎、KPI、简历/Offer、课程中心、组织架构图 |
...
| **内容门户 CMS** | 栏目 / 文章 / 评论 / 标签 / 素材库 / 表单 / SEO / Sitemap / 插件 / 敏感词 / Word 导入 / 图片裁剪 / 拼写检查 | 富文本(Tiptap)、回收站、定时发布、跨栏目复制、批量替换、导入导出 |
| **在线商城 Shop** | 商品 / 品牌 / 分类 / 优惠券 / 秒杀 / 拼团 / 砍价 / 分销 / 钱包余额 / 积分 / 仓库库存 / 快递 / 订单 / 售后 / 地区联动 | 完整电商闭环 |
| **系统** | 用户 / 角色 / 权限(细粒度) / 站点作用域 / 多语言 / 插件 / 自动化 | 权限点驱动按钮显隐、站点级 scope、中英文 138 词条
