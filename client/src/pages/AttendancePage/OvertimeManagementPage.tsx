...
interface IRecord {
  id: string; employeeId: string; employeeName?: string; overtimeType?: string;
  date?: string; startTime?: string; endTime?: string; hours?: number;
  reason?: string; status?: string; createdAt?: string; approvedBy?: string;
  approvedAt?: string; rejectReason?: string; [k: string]: any;
}
...
export default function OvertimeManagementPage() {
  ...
  const openEdit = (r: IRecord) => { setEditing(r); form.setFieldsValue({ ...r, date: r.date ? dayjs(r.date) : undefined }); setModalOpen(true); };
  ...
  const handleApprove = async (r: IRecord) => {
    try {
      const res = await fetch(`/api/${TABLE}/${r.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...r, status: 'approved', approvedAt: new Date().toISOString() }) });
  ...
  const handleReject = async (r: IRecord) => {
    Modal.confirm({
      title: '拒绝加班申请',
      content: <div><p>确定拒绝 <strong>{r.employeeName || r.employeeId}</strong> 的加班申请？</p><Input.TextArea id="om-reject-reason" rows={3} placeholder="请输入拒绝原因" /></div>,
      onOk: async () => {
        const el = document.getElementById('om-reject-reason') as HTMLTextAreaElement;
        try {
          const res = await fetch(`/api/${TABLE}/${r.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...r, status: 'rejected', rejectReason: el?.value || '', approvedAt: new Date().toISOString() }) });
  ...
  const columns = [
    ...
    { title: '加班类型', dataIndex: 'overtimeType', key: 'overtimeType', width: 110, render: (v: string) => { const t = OVERTIME_TYPES.find(x => x.value === v); return <Tag color={t?.color || 'default'}>{v || '-'}</Tag>; } },
    { title: '加班日期', dataIndex: 'date', key: 'date', width: 110, render: (v: string) => v ? String(v).slice(0, 10) : '-' },
    { title: '时长(h)', dataIndex: 'hours', key: 'hours', width: 80, align: 'center' as const, render: (v: number) => <span style={{ fontWeight: 600, color: '#fa8c16' }}>{v || '-'}</span> },
  ...
  return (
    <div style={{ padding: 24 }}>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}><Card size="small" hoverable><Statistic title="加班总时长" value={stats.totalHours} suffix="小时" prefix={<ThunderboltOutlined />} /></Card></Col>
        <Col span={6}><Card size="small" hoverable><Statistic title="工作日加班" value={stats.weekHours} suffix="小时" valueStyle={{ color: '#1890ff' }} /></Card></Col>
      </Row>
      <Card title={<span style={{ fontSize: 18, fontWeight: 600 }}>加班管理</span>} extra={<Button type="primary" icon={<PlusOutlined />} onClick={openAdd}>新增加班</Button>} styles={{ body: { padding: 0 } }}>
      ...
      <Modal title={(editing ? '编辑' : '新增') + '加班申请'} open={modalOpen} onOk={handleSave} onCancel={() => setModalOpen(false)} width={720} okText="保存" cancelText="取消">
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          <Row gutter={16}>
            ...
            <Col span={12}><Form.Item label="加班类型" name="overtimeType" rules={[{ required: true, message: '请选择' }]}><Select placeholder="请选择" options={OVERTIME_TYPES.map(t => ({ value: t.value, label: t.label }))} /></Form.Item></Col>
          </Row>
          <Row gutter={16}>
            <Col span={12}><Form.Item label="加班日期" name="date" rules={[{ required: true, message: '请选择' }]}><DatePicker style={{ width: '100%' }} /></Form.Item></Col>
            <Col span={12}><Form.Item label="加班时长(小时)" name="hours" rules={[{ required: true, message: '请输入' }]}><InputNumber style={{ width: '100%' }} min={0.5} step={0.5} /></Form.Item></Col>
          </Row>
          <Form.Item label="加班原因" name="reason" rules={[{ required: true, message: '请输入原因' }]}><Input.TextArea rows={3} placeholder="请详细说明加班原因" /></Form.Item>
      ...
      <Drawer title="加班详情" open={drawerOpen} onClose={() => setDrawerOpen(false)} width={480}
        ...
        {viewing && (
          <Descriptions column={1} bordered size="small">
            ...
            <Descriptions.Item label="加班类型"><Tag color={OVERTIME_TYPES.find(t => t.value === viewing.overtimeType)?.color || 'default'}>{viewing.overtimeType || '-'}</Tag></Descriptions.Item>
            <Descriptions.Item label="加班日期">{viewing.date ? String(viewing.date).slice(0, 10) : '-'}</Descriptions.Item>
            <Descriptions.Item label="加班时长"><span style={{ fontWeight: 600 }}>{viewing.hours || '-'}</span> 小时</Descriptions.Item>
