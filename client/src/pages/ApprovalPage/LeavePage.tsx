...
const STATUS_MAP: Record<string, { label: string; color: string; icon?: React.ReactNode }> = {
  pending: { label: '待审批', color: 'gold', icon: <ClockCircleOutlined /> },
...
export default function LeavePage() {
  ...
  const handleReject = async (r: IRecord) => {
    Modal.confirm({
      ...
      content: (
        <div>
          <p>确定拒绝 <strong>{r.employeeName || r.employeeId}</strong> 的{r.leaveType}申请？</p>
          <Input.TextArea id="reject-reason" rows={3} placeholder="请输入拒绝原因" />
        </div>
      ),
      onOk: async () => {
        const reasonEl = document.getElementById('reject-reason') as HTMLTextAreaElement;
        const rejectReason = reasonEl?.value || '';
        try {
          const res = await fetch(`/api/${TABLE}/${r.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ...r, status: 'rejected', rejectReason, approvedAt: new Date().toISOString() })
          });
          if (res.ok) {
            message.success('已拒绝');
            setDrawerOpen(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v: string) => {
        const lt = LEAVE_TYPES.find(t => t.value === v);
        return <Tag color={lt?.color || 'default'}>{v || '-'}</Tag>;
      },
    },
    {
    ...
    {
      title: '天数', dataIndex: 'days', key: 'days', width: 60, align: 'center' as const,
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* 主表格 */}
      <Card
        title={<span style={{ fontSize: 18, fontWeight: 600 }}>请假审批</span>}
        extra={
      ...
      >
        <div style={{ padding: '16px 16px 0' }}>
          <Space wrap>
            <Input.Search
              placeholder="搜索员工姓名..." value={search}
              onChange={e => setSearch(e.target.value)}
              onSearch={() => setPagination(p => ({ ...p, current: 1 }))}
              style={{ width: 220 }} allowClear
            />
            <Select
              placeholder="请假类型" value={typeFilter || undefined}
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item label="天数" name="days" rules={[{ required: true, message: '请输入天数' }]}>
                <InputNumber style={{ width: '100%' }} min={0.5} step={0.5} placeholder="选择日期后自动计算" />
              </Form.Item>
            </Col>
          </Row>
          <Row gutter={16}>
          ...
          <Form.Item label="请假原因" name="reason" rules={[{ required: true, message: '请输入请假原因' }]}>
            <Input.TextArea rows={3} placeholder="请详细说明请假原因" />
          </Form.Item>
      ...
      <Drawer
        ...
        extra={
          viewing?.status === 'pending' ? (
            <Space>
              <Button type="primary" icon={<CheckCircleOutlined />} onClick={() => viewing && handleApprove(viewing)}>
                通过
              </Button>
              <Button danger icon={<CloseCircleOutlined />} onClick={() => viewing && handleReject(viewing)}>
                拒绝
              </Button>
            </Space>
          ) : null
          ...
          <>
            <Descriptions column={1} bordered size="small">
              ...
              <Descriptions.Item label="请假类型">
                <Tag color={LEAVE_TYPES.find(t => t.value === viewing.leaveType)?.color || 'default'}>
                  {viewing.leaveType || '-'}
                </Tag>
              </Descriptions.Item>
              ...
              <Descriptions.Item label="请假天数">
                <span style={{ fontWeight: 600, fontSize: 16 }}>{viewing.days || '-'}</span> 天
              </Descriptions.Item>
              <Descriptions.Item label="状态">
                {(() => {
                  const s = STATUS_MAP[viewing.status || ''];
                  return s ? <Badge status={s.color === 'green' ? 'success' : s.color === 'red' ? 'error' : s.color === 'gold' ? 'warning' : 'default' as any} text={s.label} /> : <Tag>{viewing.status || '-'}</Tag>;
                })()}
              </Descriptions.Item>
